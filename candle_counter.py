...
class CandleCounter:
    ...
    def verify_data(self, symbol: str, timeframe: str, start_date: datetime, end_date: datetime, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if not pd.api.types.is_datetime64_any_dtype(df['timestamp']) or not pd.api.types.is_datetime64_any_dtype(df['close_time']):
                logger.error("Timestamp columns are not in datetime format")
                return False
...
def main():
    ...
    while True:
        ...
        while True:
            response = input(f"\n{Fore.YELLOW}Would you like to calculate another timeframe? (y/n): {Style.RESET_ALL}").lower()
            if response in ['y', 'n']:
                break
            print(f"{Fore.RED}Please enter 'y' or 'n'.{Style.RESET_ALL}")
    ...
    print(f"\n{Fore.GREEN}Thank you for using Binance Candle Counter!{Style.RESET_ALL}")
